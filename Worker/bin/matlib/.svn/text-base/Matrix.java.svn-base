...
public abstract class Matrix {
	 ...
	 *  Diagonal values in all rows must be set, i.e. input array must 
	 *  have same number of elements as no. of rows in this Matrix.
	 *  This Matrix must have at least as many columns as rows.
	 *  
	 * @param diagVals
	 */
	public void setDiag(double[] diagVals) {
		if (diagVals.length != this.numRows()) {
			throw new IllegalArgumentException("Input array of diagonal values must be of same"
					+ " length as number of rows in Matrix calling this method");
		}
		if (diagVals.length > this.numCols()) {
			throw new IllegalArgumentException("Input array cannot have more elements than number"
					+ " of columns in Matrix calling this method");
		}
		for (int i = 0; i < this.numRows(); ++i) {
			this.set(i, i, diagVals[i]);
		}
	}
...
//	
	...
	public void printToFile(String fname, String format) {
		...
		if (format.toUpperCase().equals("DEFAULT")) {
			try {
				...
				for (int row = 0; row < this.numRows(); ++row) {
					for (int col = 0; col < this.numCols(); ++col) {
						pw.print(this.getQuick(row, col) + " ");
					}
					pw.println();
				}
			}
   ...
   abstract public double det();
	...
	/** Sets this Matrix to diagonal Matrix containing diagValues as
	 * diagonal args
	 * 
	 * @param diagValues double array of diagonal elements
	 * @return this Matrix 
	 */
	abstract public Matrix diag(double[] diagValues);
//	{
//		if (!this.isSquare()) {
//			throw new MatrixException("This Matrix must be square");
//		}
//		
//		if (!(this.numCols() == diagValues.length)) {
//			throw new MatrixException("Input array must be length of Matrix diagonal");
//		}
//		
//		this.setIdentity();
//		for (int d = 0; d < diagValues.length; ++d) {
//			this.set(d, d, diagValues[d]);
//		}
//	}
