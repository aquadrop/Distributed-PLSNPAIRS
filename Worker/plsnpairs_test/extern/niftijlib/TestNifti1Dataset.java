...
import java.util.zip.*;
	...
	* </li>
	* <li>
	* 2/2009 AO added get methods for dims X,Y,Z,T + readDoubleVol1D(short)
	*        (written by Vicky Ning)
	* </li>
	* <li>
	* 9/2010 AO added setVoxSize(double[]) method so 3D voxel size in mm 
	*        can be set explicitly
	* </ul>
	*
	*/

public class TestNifti1Dataset {
	...
	*/
	...
	/**
	* Read one 3D volume from disk and return it as 1D double array
	* @param ttt T dimension of vol to read (0 based index)
	* @return 1D double array, scale and offset have been applied.
	* Array indices are [Z][Y][X], assuming an xyzt ordering of dimensions.
	* ie  indices are data[dim[3]][dim[2]][dim[1]]
	* @exception IOException
	* 
	*/
	public double[] readDoubleVol1D(short ttt) throws IOException {
	...
	switch (datatype) {
		...
		case NIFTI_TYPE_UINT8:
			...
			for (i=0; i<XDIM; i++) {
				...
				data[k*YDIM*XDIM+j*XDIM+i] = (double) (ecs.readByte());
				
				if ((datatype == NIFTI_TYPE_UINT8) && (data[k*YDIM*XDIM+j*XDIM+i] < 0) )
					data[k*YDIM*XDIM+j*XDIM+i] = Math.abs(data[k*YDIM*XDIM+j*XDIM+i]) + (double)(1<<7);
				if (scl_slope != 0)
					data[k*YDIM*XDIM+j*XDIM+i] = data[k*YDIM*XDIM+j*XDIM+i] * scl_slope + scl_inter;
				//System.out.println(data[k*YDIM*XDIM+j*XDIM+i]);
			}
		...
		case NIFTI_TYPE_UINT16:
			...
			for (j=0; j<YDIM; j++)
			for (i=0; i<XDIM; i++) {
				data[k*YDIM*XDIM+j*XDIM+i] = (double) (ecs.readShortCorrect());
				
				if ((datatype == NIFTI_TYPE_UINT16) && (data[k*YDIM*XDIM+j*XDIM+i] < 0))
					data[k*YDIM*XDIM+j*XDIM+i] = Math.abs(data[k*YDIM*XDIM+j*XDIM+i]) + (double)(1<<15);
				if (scl_slope != 0)
					data[k*YDIM*XDIM+j*XDIM+i] = data[k*YDIM*XDIM+j*XDIM+i] * scl_slope + scl_inter;
		...
		case NIFTI_TYPE_UINT32:
			...
			for (j=0; j<YDIM; j++)
			for (i=0; i<XDIM; i++) {
				data[k*YDIM*XDIM+j*XDIM+i] = (double) (ecs.readIntCorrect());
				if ( (datatype == NIFTI_TYPE_UINT32) && (data[k*YDIM*XDIM+j*XDIM+i] < 0) )
					data[k*YDIM*XDIM+j*XDIM+i] = Math.abs(data[k*YDIM*XDIM+j*XDIM+i]) + (double)(1<<31);
				if (scl_slope != 0)
					data[k*YDIM*XDIM+j*XDIM+i] = data[k*YDIM*XDIM+j*XDIM+i] * scl_slope + scl_inter;
		...
		case NIFTI_TYPE_UINT64:
			...
			for (j=0; j<YDIM; j++)
			for (i=0; i<XDIM; i++) {
				data[k*YDIM*XDIM+j*XDIM+i] = (double) (ecs.readLongCorrect());
				if ( (datatype == NIFTI_TYPE_UINT64) && (data[k*YDIM*XDIM+j*XDIM+i] < 0) )
					data[k*YDIM*XDIM+j*XDIM+i] = Math.abs(data[k*YDIM*XDIM+j*XDIM+i]) + (double)(1<<63);
				if (scl_slope != 0)
					data[k*YDIM*XDIM+j*XDIM+i] = data[k*YDIM*XDIM+j*XDIM+i] * scl_slope + scl_inter;
		...
		case NIFTI_TYPE_FLOAT32:
			...
			for (j=0; j<YDIM; j++)
			for (i=0; i<XDIM; i++) {
				data[k*YDIM*XDIM+j*XDIM+i] = (double) (ecs.readFloatCorrect());
				if (scl_slope != 0)
					data[k*YDIM*XDIM+j*XDIM+i] = data[k*YDIM*XDIM+j*XDIM+i] * scl_slope + scl_inter;
		...
		case NIFTI_TYPE_FLOAT64:
			...
			for (j=0; j<YDIM; j++)
			for (i=0; i<XDIM; i++) {
				data[k*YDIM*XDIM+j*XDIM+i] = (double) (ecs.readDoubleCorrect());
				if (scl_slope != 0)
					data[k*YDIM*XDIM+j*XDIM+i] = data[k*YDIM*XDIM+j*XDIM+i] * scl_slope + scl_inter;
	...
	//System.out.println(StatUtils.min(data));
	...
	*/
	...
	/** Sets voxel size (3D) to given size in mm.
	 * 
	 * @param voxSize 3-element array giving {Xsize, Ysize, Zsize} in mm.
	 *        If Xsize is negative, affine data orientation will be R-to-L
	 *        If Ysize is negative, affine data orientation will be A-to-P
	 *        If Zsize is negative, affine data orientation will be S-to-I.
	 *        Else affine (sform) data orientation will be RAS by default.
	 */
	public void setVoxSize(double[] voxSize) {
		if  (voxSize.length != 3) {
			throw new IllegalArgumentException("Invalid size of input array; expecting" +
					"array with 3 elements.");
		}
		pixdim[1] = (float)voxSize[0];
		pixdim[2] = (float)voxSize[1];
		pixdim[3] = (float)voxSize[2];
	}
	...
	{
		return new double[]{(double)pixdim[1],(double)pixdim[2],(double)pixdim[3]};
	}
