 ...
 */
public class NpairsBoxAndWhiskerRenderer extends AbstractCategoryItemRenderer
     ...
     * Default constructor.
     */
    public NpairsBoxAndWhiskerRenderer() {
...
//                               + (seriesGap * (seriesCount - 1));
            double usedWidth = (widthWeWannaUse * seriesCount)
            + (seriesGap * (seriesCount - 1));
...
//                    Math.abs(xxQ1 - xxQ3), state.getBarWidth());
            box = new Rectangle2D.Double(Math.min(xxQ1, xxQ3), yy,
                    Math.abs(xxQ1 - xxQ3), widthWeWannaUse);
            if (this.fillBox) {
                g2.fill(box); 
            }
    ...
    public void drawVerticalItem(Graphics2D g2,
        ...
        if (seriesCount > 1) {
            double seriesGap = dataArea.getWidth() * getItemMargin()
                               / (categoryCount * (seriesCount - 1));
            double usedWidth = (widthToUse * seriesCount)
                               + (seriesGap * (seriesCount - 1));
            // offset the start of the boxes if the total width used is smaller
            // than the category width
            double offset = (categoryWidth - usedWidth) / 2;
        ...
        if (yQ1 != null && yQ3 != null && yMax != null && yMin != null) {
            ...
            g2.draw(new Line2D.Double(xx, yyMax, xx + widthToUse,
                    yyMax));

            // draw the lower shadow...
            g2.draw(new Line2D.Double(xxmid, yyMin, xxmid, yyQ1));
            g2.draw(new Line2D.Double(xx, yyMin, xx + widthToUse,
                    yyMin));
...
//            aRadius = widthWeWannaUse / 4;
        ...
        if (yMedian != null && !Double.isNaN(yMedian.doubleValue())) {
            double yyMedian = rangeAxis.valueToJava2D(yMedian.doubleValue(),
                    dataArea, location);
            g2.draw(new Line2D.Double(xx, yyMedian, xx + widthToUse,
                    yyMedian));
...
//            }
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof NpairsBoxAndWhiskerRenderer)) {
            return false;
        }
        if (!super.equals(obj)) {
            return false;
        }
        NpairsBoxAndWhiskerRenderer that = (NpairsBoxAndWhiskerRenderer) obj;
