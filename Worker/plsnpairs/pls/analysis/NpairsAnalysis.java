...
import rmi.Information;
import rmi.NpairsDataLoaderContainer;
...
public class NpairsAnalysis extends ProgressDialogWatcher {
	...
	int firstAnalysis;     // for rmi implementation
	int lastAnalysis;      // for rmi implementation
	boolean server;        // for rmi implementation
	
	
	//public static ResultSaverContainer container = null;
	...
	public NpairsAnalysis(String npairsSetupParamsMatFileName, String matlibType, 
			String matlibTypeForInitFeatSel, boolean isBlocked, boolean evdOnly, int analysisNum, int start, int end) {
		this.npairsSetupParamsMatFileName = npairsSetupParamsMatFileName;
		this.matlibType = matlibType;
		this.matlibTypeForInitFeatSel = matlibTypeForInitFeatSel;
		this.isBlocked = isBlocked;
	...
	public void doTask() throws Exception {
		...
		NpairsjSetupParams nsp = null;
		
			nsp = new NpairsjSetupParams(npairsSetupParamsMatFileName, !isBlocked);
		    ...
		    if (!resultsDir.exists()) {
		     ...
		     else 
		     {
				    ...
				    if (!(evdOnly && loadEVD)) {
			    		...
			    		if (!NpairsDataLoaderContainer.avoid_load_sign)
						{
			    			System.out.println("##############True Loading###############");
			    			ndl = new NpairsDataLoader(nsp, matlibType, 
						    		matlibTypeForInitFeatSel, analysisType.equals("event-related"));
							NpairsDataLoaderContainer.ndl = ndl;
							NpairsDataLoaderContainer.avoid_load_sign = true;
							//NpairsDataLoaderContainer.nsp = nsp;
						}
						else
						{
							System.out.println("##############False Loading#############");
							ndl = NpairsDataLoaderContainer.ndl;
						}
			...
			if(server)// indicate run by rmi
			{
				
				firstAnalysisIdx = this.firstAnalysis;
				lastAnalysisIdx = this.lastAnalysis;
//				System.out.println("firstAnalysis:"+firstAnalysisIdx);
//				System.out.println("lastAnalysis:"+lastAnalysisIdx);
			}
//			System.out.println("firstAnalysis:"+firstAnalysisIdx);
...
//                        container = new ResultSaverContainer(npairsj, 
//                        		npairsSetupParamsMatFileName, 
//                        		nsp.resultsFilePrefix);
//                       // container.translate(npairsj);
//                        System.out.println("Transmit Complete");
////						Thread transmitthread = new Thread(new ObjectDeliver(container));
