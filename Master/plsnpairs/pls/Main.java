...
import rmi.MasterReceiver;
import rmi.MasterRunReceiver;
import rmi.MainInterface;
import rmi.NpairsAnalysisShell;
...
public class Main {
	...
	public Main(boolean mastermode)
	{
		Main.mastermode=mastermode;
	}
...
//				}
	...
	private static void runAnalysis(String[] argv, boolean isNpairs) throws Exception {
		...
		for(int n = 2; n <= lastIdx; ++n) {
			...
			if (!isNpairs) {
				...
				{
					setMasterMode();
					//shell to create directory and get analysis number
					NpairsAnalysisShell worker = new NpairsAnalysisShell(filename, matlibType, 
							matlibTypeForInitFeatSel, !loadDatamats, evdOnly, analysisNum);
					
					
					worker.progress = helper;
					worker.run();
					int npairsanalysisNum=NpairsAnalysisShell.getNumNPAIRS();
					System.out.println("numNPAIRS: "+ npairsanalysisNum);
					MasterReceiver master = new MasterReceiver(npairsanalysisNum, argv);
					MasterRunReceiver.totalAnalysis=npairsanalysisNum;
					MasterRunReceiver.initialQueue();
					master.run();
				}
//				
...
//	private static String getBroadCastIP() throws RemoteException
//	{
//		String address = null;
//		try
//		{
